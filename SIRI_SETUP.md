# Siri Time Tracking Setup (Telegram Native)

## Best Solution: Use Telegram's Native iOS Integration

Send commands directly via Telegram app using iOS Shortcuts.
...
2. **Shortcuts** app
3. **DM with your bot** or dedicated topic in a group
...
4. Add action: **Send Message** (Telegram)
   - Recipient: Select your bot DM or time tracking topic
   - Message: `start: ` then add **Provided Input** variable
...
2. Add action: **Send Message** (Telegram)
   - Recipient: Your bot DM or time tracking topic
   - Message: `stop`
...
3. Add action: **Send Message** (Telegram)
   - Recipient: Your bot DM or time tracking topic
   - Message: `time: ` then add **Provided Input** variable
...
Same as Stop Timer, but message is: `report today`
...
1. Open DM with your bot (or time tracking topic)
2. Manually type: `start: testing`
3. Wait a few seconds, type: `stop`
4. Bot should respond with confirmation and duration
...
⚠️ **Topic Selection:** iOS Shortcuts might not support Telegram topics directly.

**Workaround:**
- Create a dedicated **DM** with your bot (just you + bot)
- Use that for all time tracking
